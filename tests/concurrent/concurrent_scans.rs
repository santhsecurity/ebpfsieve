...
fn test_concurrent_independent_filters() {
    ...
    for i in 0..5 {
        ...
        handles.push(thread::spawn(move || {
            b.wait();
            let byte = b'a' + i;
            let filter = ByteFrequencyFilter::new([ByteThreshold::new(byte, 2)])
                .unwrap()
                .with_window_size(5)
                .unwrap();
            ...
            assert_eq!(
                matches.len(),
                6,
                "Expected 6 matches for independent filter {}",
                i
            );
        }));
    }
...
fn test_concurrent_writes_to_shared_state() {
    let filter = Arc::new(
        ByteFrequencyFilter::new([ByteThreshold::new(b'x', 3)])
            .unwrap()
            .with_window_size(5)
            .unwrap(),
    );
...
fn test_concurrent_read_write_large_data() {
    ...
    let filter = Arc::new(
        ByteFrequencyFilter::new([ByteThreshold::new(b'x', 5)])
            .unwrap()
            .with_window_size(10)
            .unwrap(),
    );
...
fn test_concurrent_attachments() {
    let filter = Arc::new(
        ByteFrequencyFilter::new([ByteThreshold::new(b'x', 2)])
            .unwrap()
            .with_window_size(4)
            .unwrap(),
    );
    ...
    for _ in 0..10 {
        ...
        handles.push(thread::spawn(move || {
            ...
            assert_eq!(
                matches.len(),
                5,
                "Concurrent attachment scans should yield correct results"
            );
        }));
    }
