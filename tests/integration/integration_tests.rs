...
fn test_multiple_filters_same_file() {
    ...
    file.write_all(b"hello world, this is a very long string used for testing ebpfsieve functionality.").unwrap();
    
    let filter1 = ByteFrequencyFilter::new([ByteThreshold::new(b'e', 2)]).unwrap().with_window_size(20).unwrap();
    let filter2 = ByteFrequencyFilter::new([ByteThreshold::new(b'o', 2)]).unwrap().with_window_size(20).unwrap();
    ...
    assert_ne!(matches1.len(), matches2.len(), "Different thresholds should likely yield different match counts");
}

#[test]
fn test_chunk_size_alignment_issues() {
    let mut file = NamedTempFile::new().unwrap();
    file.write_all(b"1234567890abcdefghijklmnopqrstuvwxyz").unwrap();
    
    let filter = ByteFrequencyFilter::new([ByteThreshold::new(b'a', 1)]).unwrap()
        .with_window_size(5).unwrap()
        .with_chunk_size(3).unwrap();
        
    let mut f = File::open(file.path()).unwrap();
    let matches = filter.scan_file(&mut f, None).unwrap();
    
    assert_eq!(matches.len(), 5, "Should find exactly 5 matches for 'a' with window 5");
}
...
fn test_seek_and_scan() {
    ...
    let filter = ByteFrequencyFilter::new([ByteThreshold::new(b'x', 4)]).unwrap().with_window_size(4).unwrap();
    
    let mut f = File::open(file.path()).unwrap();
    f.seek(SeekFrom::Start(10)).unwrap();
    
    let matches = filter.scan_file(&mut f, None).unwrap();
    assert_eq!(matches.len(), 1, "Should find the match right at the start of scan");
    assert_eq!(matches[0].offset, 0, "Matches are relative to scan start");
}

#[test]
fn test_interrupted_reads() {
    let filter = ByteFrequencyFilter::new([ByteThreshold::new(b'a', 3)]).unwrap().with_window_size(5).unwrap();
    let data = b"aaaaa";
    ...
    assert!(matches.len() > 0, "Should correctly assemble matches across 1-byte reads");
}
...
fn test_extremely_large_file_scan_with_limit() {
    ...
    assert!(matches.len() > 1_000_000, "Should find many matches in the 2MB segment");
}
