...
fn e2e_scan_large_dataset() {
    ...
    ]).unwrap().with_window_size(1000).unwrap();
    
    let matches = filter.matching_windows(&data);
    assert!(matches.len() > 0, "Should find matches in the cyclic buffer");
}
...
fn e2e_scan_sparse_dataset() {
    ...
    let matches = filter.matching_windows(&data);
    assert_eq!(matches.len(), 8, "Exact match count in sparse data should be predictable");
}
