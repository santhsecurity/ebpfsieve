...
fn test_extremely_large_window_with_small_threshold() {
    ...
    let matches = filter.matching_windows(&data);
    assert!(!matches.is_empty(), "Should handle usize::MAX window with matching data correctly");
}
...
fn test_duplicate_thresholds_for_same_byte() {
    ...
    assert!(matches_fail.is_empty(), "Should use the highest requirement, so 4 is not enough for 5");
...
fn test_mixed_null_bytes_and_newlines() {
    ...
    let data = b"abc\n\0\0def\0\nghi";
    let matches = filter.matching_windows(data);
    assert!(!matches.is_empty(), "Should match binary data mixed with newlines");
}
