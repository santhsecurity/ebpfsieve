...
fn test_large_input_fuzzing() {
    let filter = ByteFrequencyFilter::new([ByteThreshold::new(b'x', 1)]).unwrap().with_window_size(10).unwrap();
    let data = vec![b'x'; 10_000];
    let matches = filter.matching_windows(&data);
    assert_eq!(matches.len(), 10000 - 10 + 1, "Should find large matches safely");
}
...
fn test_extremely_large_max_matches() {
    ...
    let data = vec![b'x'; 10_000];
    let matches = filter.matching_windows(&data);
    assert_eq!(matches.len(), 10_000, "Should handle usize::MAX max_matches safely");
}
...
fn test_all_thresholds_zero_except_one() {
    ...
    let filter = ByteFrequencyFilter::new(thresholds).unwrap().with_window_size(10).unwrap();
    ...
    let matches = filter.matching_windows(&data);
    assert_eq!(matches.len(), 10, "Should find the exact windows containing byte 42");
}

#[test]
fn test_alternating_bytes_dense() {
    let filter = ByteFrequencyFilter::new([
        ByteThreshold::new(b'0', 5),
        ByteThreshold::new(b'1', 5),
    ]).unwrap().with_window_size(10).unwrap();
    
    let data = b"01010101010101010101"; // 20 bytes
    let matches = filter.matching_windows(data);
    assert_eq!(matches.len(), 11, "Alternating bytes should produce 11 matches");
}
...
fn test_extremely_large_chunk_sizes() {
    ...
    assert_eq!(attachment.filter().chunk_size(), usize::MAX, "Filter created with extreme chunk size");
}
...
fn test_maximum_possible_threshold() {
    ...
    let matches = filter.matching_windows(&data);
    assert!(matches.is_empty(), "Small data should safely fail large threshold");
}
...
fn test_all_possible_byte_thresholds() {
    ...
    let filter = ByteFrequencyFilter::new(thresholds).unwrap().with_window_size(256).unwrap();
    ...
    let matches = filter.matching_windows(&data);
    assert_eq!(matches.len(), 1, "Exactly one window contains all bytes exactly once");
}
